...
def runScraperForAuthor(authorId, url, headless): 

    print('Starting to scrape reviews for author', authorId)
    ...
    for reviewBlob in reviewBlobs:
        ...
        if len(review_objects) % 50 == 0:
            print('Found', len(review_objects), 'reviews for author', authorId)
    ...
    print('Found', numReviewsFound, 'reviews for author', authorId, 'Finished scraping author.')
